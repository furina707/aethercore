...
DIRECT_DOMAINS = [
    # 腾讯系
    "qq.com", "tencent.com", "music.tc.qq.com", "y.qq.com", "gtimg.com", "qpic.cn", "weixin.qq.com",
    # 网易系
    "163.com", "126.net", "music.163.com", "netease.com", "126.com", "ydstatic.com",
    # 音视频
    "kugou.com", "kuwo.cn", "migu.cn", "bilibili.com", "bilivideo.com", "biliapi.net", "hdslb.com",
    "iqiyi.com", "youku.com", "mgtv.com", "ximalaya.com", "kuaishou.com",
    # 百度 / 阿里
    "baidu.com", "baidubce.com", "bcebos.com", "bdimg.com",
    "alipay.com", "taobao.com", "tmall.com", "jd.com", "aliyun.com", "alicdn.com", "alibabacloud.com",
    # 华为与华为云 (解决海外节点拦截与 ERR_CONNECTION_CLOSED)
    "huawei.com", "huaweicloud.com", "myhuaweicloud.com", "hicloud.com", "dbankcdn.com", "vmall.com",
    # 字节跳动 / 小米 / 美团
    "bytedance.com", "douyin.com", "toutiao.com", "feishu.cn", "larksuite.com", "pstatp.com", "volces.com",
    "xiaomi.com", "mi.com", "meituan.com", "dianping.com", "pinduoduo.com", "yangkeduo.com",
    # 国内云平台与 CDN
    "tencentcloud.com", "myqcloud.com", "qiniu.com", "qiniucdn.com", "upyun.com", "ucloud.cn",
    # 开源镜像与系统更新
    "ubuntu.com", "canonical.com", "debian.org", "archlinux.org", "centos.org", "alpinelinux.org",
    "fedoraproject.org", "opensuse.org", "kernel.org", "gnu.org", "tsinghua.edu.cn", "ustc.edu.cn",
    # 开发者与社区
    "gitee.com", "csdn.net", "oschina.net", "v2ex.com", "cnblogs.com",
    # 顶级域
    "cn",
]
...
def fetch_subscription(cache_path: str) -> bool:
    ...
    try:
        rc = subprocess.run(cmd, capture_output=True, timeout=12)
        if rc.returncode == 0 and os.path.exists(cache_path) and os.path.getsize(cache_path) > 0:
            return True
    except Exception:
        pass

    # 若直连拉取失败，仅当本地代理端口 7899/7897 处于存活监听状态时才尝试代理重试，避免无效等待挂起
    for port in (7899, 7897):
        try:
            ...
            if r == 0:
                cmd_proxy = list(cmd)
                cmd_proxy[1:1] = ["-x", f"http://127.0.0.1:{port}"]
                rc = subprocess.run(cmd_proxy, capture_output=True, timeout=8)
                if rc.returncode == 0 and os.path.exists(cache_path) and os.path.getsize(cache_path) > 0:
                    return True
        except Exception:
            pass
...
def generate(data_dir: str, mode: str = "auto") -> bool:
    ...
    if mode != "rebuild":
        ...
        if fetch_subscription(cache_path):
            ...
            if (os.path.exists(cache_path) and os.path.getsize(cache_path) > 0) or (os.path.exists(alt_path) and os.path.getsize(alt_path) > 0):
                print("[!] 订阅实时拉取失败或超时，自动使用本地历史缓存配置启动...")
            else:
                print("[x] 订阅拉取失败且无历史缓存")
                return False
    ...
    # 排除日本节点（名称含"日本"或🇯🇵）
    EXCLUDE_KEYWORDS = ["日本", "\U0001f1ef\U0001f1f5"]  # 🇯🇵
    before_count = len(nodes)
    nodes = [n for n in nodes if not any(kw in n.name for kw in EXCLUDE_KEYWORDS)]
    excluded = before_count - len(nodes)
    if excluded:
        print(f"[*] 已排除 {excluded} 个日本节点，剩余 {len(nodes)} 个节点")
